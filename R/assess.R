...
RCSmixed.fit_initial=function(mod,dat,resp_var,tol=0.1)
{
  ...
  #cat("det(G_hat) = ", (determinant(G_hat[n_t != 0, n_t != 0], logarithm = TRUE)$modulus |> as.numeric()), "\n")
  ...
  while (abs(l_old-l_new) > tol){
    ...
    V = f_V(n_t)
    dat$less_u_hat = dat[[resp_var]] - as.vector(X_t[,n_t != 0] %*% u_hat)
    #print(head(dat$less_u_hat))
    new_formula = formula(mod)
    new_formula[[2]] = quote(less_u_hat)
    re_new = lme4::lmer(data = dat, formula = new_formula)
    #print(summary(re_new))
    l1 = re_new |> logLik() |> as.numeric() #This is the likelihood of the fitted model after subtracting the time series
    cat("new model log(lik) = ", l1, "\n")
    X_t_sparse = Matrix(X_t)
    X_r_sparse = Matrix(X_r)

    G_inv = G_hat |> chol() |> chol2inv()
    Gplus_inv = chol2inv(chol((G_inv + N / var_eps_hat)))
    B_inv = (solve(var_re) + Matrix::t(X_r) %*% X_r / var_eps_hat - var_eps_hat ^ (-2) * Matrix::t(X_r) %*% X_t_sparse %*% Gplus_inv %*% Matrix::t(X_t_sparse) %*% X_r_sparse) |> chol() |> chol2inv()
    R1 = t(X_f_m) %*% X_f_m / var_eps_hat - var_eps_hat ^ (-2) * (t(X_f_m) %*% X_t) %*% chol2inv(chol((G_inv + N / var_eps_hat))) %*% (t(X_t) %*% X_f_m)
    Bchol = B_inv |> chol() |> t()

    R2_half = t(X_f_m) / var_eps_hat - var_eps_hat ^ (-2) * t(X_f_m) %*% X_t_sparse %*% Gplus_inv %*% Matrix::t(X_t_sparse)  #%*% AINVERSE %*% X_r_sparse %*% Bchol
    #R2 = R2_half %*% t(R2_half)
    R2_half = R2_half %*% X_r_sparse %*% t(chol(B_inv))
    R2 = R2_half %*% t(R2_half)

    R = R1 + R2
    R_inv = R |> chol() |> chol2inv()
    S1 = y / var_eps_hat - as.vector(var_eps_hat ^ (-2) * (X_t_sparse %*% ((Gplus_inv) %*% (Matrix::t(X_t_sparse) %*% y))))
    S2 = Matrix::t(X_r_sparse) %*% S1
    S2 = B_inv %*% S2
    S2 = X_r_sparse %*% S2
    S2 = S2 / var_eps_hat - var_eps_hat^(-2) * (X_t_sparse %*% ((Gplus_inv) %*% (Matrix::t(X_t_sparse) %*% S2)))
    S = S1 - S2
    beta_hat = R_inv %*% t(X_f_m) %*% S
  ...
  beta_hat = cbind(beta_hat, sqrt(diag(Sigma_beta)))
  pvals=2*(1-pnorm(abs(beta_hat[,1]/beta_hat[,2])))
  beta_hat=cbind(beta_hat,pvals)
